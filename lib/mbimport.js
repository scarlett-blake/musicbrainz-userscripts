...
var MBImport = (function () {
    ...
    function fnBuildFormParameters(release, edit_note) {
        ...
        // Date + country
        if (Array.isArray(release.country)) {
            for (let i = 0; i < release.country.length; i++) {
                let country = release.country[i];
                appendParameter(parameters, `events.${i}.country`, country.country);
                if (!isNaN(country.year) && country.year != 0) {
                    appendParameter(parameters, `events.${i}.date.year`, country.year);
                }
                if (!isNaN(country.month) && country.month != 0) {
                    appendParameter(parameters, `events.${i}.date.month`, country.month);
                }
                if (!isNaN(country.day) && country.day != 0) {
                    appendParameter(parameters, `events.${i}.date.day`, country.day);
                }
            }
        } else {
